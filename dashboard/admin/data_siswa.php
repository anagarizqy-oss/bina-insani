...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        // Get user_id first
        $stmt_get = $pdo->prepare("SELECT user_id FROM siswa WHERE id = ?");
        $stmt_get->execute([$id]);
...
<body>
    ...
    <div class="main-content">
        <header style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center;">
            ...
            <div class="header-actions" style="display: flex; gap: 10px;">
                <!-- Export -->
                <a href="function_admin/export_siswa.php" class="btn-export" style="background: #4caf50; color: white; padding: 10px 15px; text-decoration: none; border-radius: 6px; font-weight: 500; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 5px;">
                ...
                <a href="function_admin/add_siswa.php" class="btn-add" style="background: #2575fc; color: white; padding: 10px 15px; text-decoration: none; border-radius: 6px; font-weight: 500; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 5px;">
                        ...
                        <tbody>
                            ...
                            foreach ($siswa_list as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="function_admin/add_siswa.php?id=<?= $row['id'] ?>" style="color: #2575fc; margin-right: 10px;"><i class="fas fa-edit"></i></a>
                                        <a href="?delete=<?= $row['id'] ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus data siswa ini? Akun login juga akan dihapus.')" style="color: #e53935;"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
    ...
    <div id="importModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 1000;">
        <div style="background: white; padding: 2rem; border-radius: 12px; width: 400px; max-width: 90%;">
            <h3 style="margin-bottom: 1rem;">Import Data Siswa (Excel/CSV)</h3>
            <p style="font-size: 0.9rem; color: #666; margin-bottom: 1rem;">
                Format: NIS, Nama, Kelas, Jurusan, NoKelas, Absen, HP.<br>
                Mendukung file .xlsx, .xls, dan .csv.<br>
                Baris pertama header (akan diabaikan).
            </p>
            <form action="function_admin/import_siswa.php" method="POST" enctype="multipart/form-data">
